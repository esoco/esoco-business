...
//			rCommandLine.requireOption(ARG_RENDERER).toString();
		...
		for (String sProcess : aProcesses)
		{
			try
			{
				...
				{
					throw new CommandLineException(String.format("Class is not a process definition: %s",
																 sProcess),
												   ARG_PROCESSES);
				}
			}
			...
			{
				throw new CommandLineException(String.format("Could not create process definition for class %s",
															 sProcess),
											   ARG_PROCESSES,
											   e);
			}
	...
	{
		rOutput.printf("Usage: %s -%s <render service URL> -%s <process definition classes> -port <listening port>\n",
