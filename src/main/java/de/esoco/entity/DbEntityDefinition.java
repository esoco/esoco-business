...
public class DbEntityDefinition extends EntityDefinition<Entity>
{
	 ...
	 *                          default URL
	 * @param bAutogeneratedKey TRUE to mark primary key fields as automatically
	 *                          generated
	 */
	DbEntityDefinition(String  sEntityName,
					   String  sIdPrefix,
					   String  sTableName,
					   String  sJdbcUrl,
					   boolean bAutogeneratedKey)
	{
		init(sEntityName,
			 sIdPrefix,
			 Entity.class,
													  ...
													  int    nDatatype)
	{
		...
		if (rDatatype == null)
		{
			throw new IllegalArgumentException("No datatype mapping for " +
											   sColumn);
		}

		return RelationTypes.newRelationType(sTable.toLowerCase() + "." +
											 sColumn.toUpperCase(),
											 rDatatype);
	}
	...
	private static Connection openConnection(String sJdbcUrl)
	{
		...
		{
			throw new IllegalArgumentException("Could not open connection: " +
											   sUrl);
		}
	 ...
	 */
	private static List<RelationType<?>> readAttributes(
		String  sJdbcUrl,
		String  sTableName,
		boolean bAutogeneratedKey)
	{
		...
		{
			throw new IllegalArgumentException("Could not read metadata for " +
											   sTableName);
		}
		finally
		{
