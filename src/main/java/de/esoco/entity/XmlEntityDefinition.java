...
public class XmlEntityDefinition extends EntityDefinition<Entity>
{
	...
	private static class XmlHandler extends DefaultHandler
	{
							   ...
							   String qName)
		{
			...
			// If bCheck is false at this point, two endElements follow one another, which means that an
			...
			if (!bCheck)
			{
				...
				while (true)
				{
					// iterate backwards thru the whole list of elements until the current element is equal
					// to an element in the list.
					if (!aElementsList.get(nListSize - 1).equals(qName))
					{
						Set<String> aSet = new HashSet<String>();

						// go thru the set of already found entities and check if an entity is contained in the List
						for (String sEntity : aEntitySet)
						{
							if (sEntity.equals(aElementsList.get(nListSize -
																 1)))
							{
